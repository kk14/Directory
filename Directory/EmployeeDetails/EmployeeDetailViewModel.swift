...
class EmployeeDetailViewModel {
    ...
    init(withEmployee employee: Employee) {
        ...
        imageService = ImageServiceImplementation(api: API(urlSession: URLSession(configuration: URLSessionConfiguration.default), baseURL: URL(string: employee.avatar)!))
        employeeAvatarImage = Observable<UIImage>(UIImage(named: "person") ?? #imageLiteral(resourceName: "person"))
        setupAllDetails()
        getEmployeeAvatarImage()
    }
